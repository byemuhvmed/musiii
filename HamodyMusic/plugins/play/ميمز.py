...
from HahamodyMusic import app


@app.on_message(filters.command(["صرصار"], ""))
def hamody(client, message):
    message.reply_photo(
        photo="https://graph.org/file/0331103b1c119716bad44.jpg",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("《🪳》", callback_data="hamody")]])
    )

@app.on_callback_query(filters.command("hamody"))
def hamodyy(client, callback_query):
...
def zeqe(client, message):
    message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("《🐖》", callback_data="hamodyl")]])
    )

@app.on_callback_query(filters.command("hamodyl"))
...
def namlo(client, message):
    message.reply_photo(
        ...
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("《🦗》", callback_data="hamodyll")]])
    )

@app.on_callback_query(filters.command("hamodyll"))
